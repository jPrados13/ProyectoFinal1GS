	...
	<div class="mainContainer">
		...
		<%for(int i = 0; i < locales.size(); i++) { %>
			<div class="card" style="width: 18rem;">
			  <img class="card-img-top" src="images/locales/<%=locales.get(i).getTelefono()%>.jpg" alt="Foto del local">
			  <div class="card-body">
			    <h5 class="card-title"><%=locales.get(i).getTelefono()%></h5>
			    <p class="card-text"><%=locales.get(i).getCalle()%></p>
			    <form action="modificarLocal.jsp" method="post">
			    <input type="hidden" class="option" name="id_local" value="<%=locales.get(i).getId_local()%>"/>
			     <button type="submit" class="btn btn-primary">Modificar</button>
			     </form>
			     <hr>
			     <form action="operaciones.jsp?tipo=bajaLocal" method="post">
			    <input type="hidden" class="option" name="id_local" value="<%=locales.get(i).getId_local()%>"/>
