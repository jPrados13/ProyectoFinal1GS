...
<% ArrayList<Trabajador> trabajadores = controladorBD.dameTrabajadores();%>
<!DOCTYPE html>
<html>
<head>
	<title>Nuestros Trabajadores</title>
	...
	<div class="mainContainer">
		...
		<%for(int i = 0; i < trabajadores.size(); i++) { %>
			<div class="card" style="height: 250px !important; width: 18rem;">
			  <div class="card-body">
			    <h5 class="card-title"><%=trabajadores.get(i).getNombre_trabajador()%></h5>
			    <p class="card-text"><%=trabajadores.get(i).getPuesto()%></p>
			    <form action="modificarTrabajador.jsp" method="post">
			    <input type="hidden" class="option" name="dni" value="<%=trabajadores.get(i).getDni_trabajador()%>"/>
			     <button type="submit" class="btn btn-primary">Modificar</button>
			     </form>
			     <hr>
			     <form action="operaciones.jsp?tipo=bajaTrabajador" method="post">
			    <input type="hidden" class="option" name="dni" value="<%=trabajadores.get(i).getDni_trabajador()%>"/>
