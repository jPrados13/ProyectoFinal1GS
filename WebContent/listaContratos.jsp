...
<% ArrayList<Contrato> contratos = controladorBD.dameContratos();%>
<% String currentPart = ""; %>
<% String currentVol = ""; %>
<!DOCTYPE html>
<html>
...
<body>
	...
	<div class="mainContainer">
		...
		<%for(int i = 0; i < contratos.size(); i++) { 
			currentPart = controladorBD.dameNombreParticular(contratos.get(i).getDni_Particular());
			currentVol = controladorBD.dameNombreVoluntario(contratos.get(i).getDni_Voluntario());%>
			<div class="card" style="height: 300px !important; width: 18rem;">
			  <div class="card-body">
			    <h5 class="card-title"><%=contratos.get(i).getCodigo_visita()%></h5>
			    <p class="card-text"><%=currentPart + " - " + currentVol%>
			    <br>
			    <%=contratos.get(i).getFecha_inicio() + " - " + contratos.get(i).getFecha_fin()%><br><%=contratos.get(i).getPresupuesto() + "€"%> </p>
			    <form action="modificarContrato.jsp" method="post">
			    <input type="hidden" class="option" name="codigo" value="<%=contratos.get(i).getCodigo_visita()%>"/>
			     <button type="submit" class="btn btn-primary">Modificar</button>
			     </form>
			     <hr>
			     <form action="operaciones.jsp?tipo=bajaContrato" method="post">
			    <input type="hidden" class="option" name="codigo" value="<%=contratos.get(i).getCodigo_visita()%>"/>
