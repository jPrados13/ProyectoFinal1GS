	...
	<div class="mainContainer">
		...
		<%for(int i = 0; i < socios.size(); i++) { %>
			<div class="card" style="height: 250px !important; width: 18rem;">
			  <div class="card-body">
			    <h5 class="card-title"><%=socios.get(i).getNombre_Socio()%></h5>
			    <p class="card-text"><%=socios.get(i).getTelefono_socio()%></p>
			    <form action="modificarSocio.jsp" method="post">
			    <input type="hidden" class="option" name="dni" value="<%=socios.get(i).getDni_Socio()%>"/>
			     <button type="submit" class="btn btn-primary">Modificar</button>
			     </form>
			     <hr>
			     <form action="operaciones.jsp?tipo=bajaSocio" method="post">
			    <input type="hidden" class="option" name="dni" value="<%=socios.get(i).getDni_Socio()%>"/>
