...
<html>
<head>
	<title>Nuestros Veterinarios</title>
	...
	<div class="mainContainer">
		...
		<%for(int i = 0; i < veterinarios.size(); i++) { %>
			<div class="card" style="height: 250px !important; width: 18rem;">
			  <div class="card-body">
			    <h5 class="card-title"><%=veterinarios.get(i).getNombre_veterinario()%></h5>
			    <p class="card-text"><%=veterinarios.get(i).getTelefono_veterinario()%></p>
			    <form action="modificarVeterinario.jsp" method="post">
			    <input type="hidden" class="option" name="dni" value="<%=veterinarios.get(i).getDni_veterinario()%>"/>
			     <button type="submit" class="btn btn-primary">Modificar</button>
			     </form>
			     <hr>
			     <form action="operaciones.jsp?tipo=bajaVeterinario" method="post">
			    <input type="hidden" class="option" name="dni" value="<%=veterinarios.get(i).getDni_veterinario()%>"/>
